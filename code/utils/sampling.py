...
def my_noniid(dataset, args):
    ...
    dict_users = {i: np.array([], dtype='int64') for i in range(args.total_users)}
    
    
    noniid_img_per_local = int(54000//args.total_users*args.noniid)
    iid_img_per_local = int(54000//args.total_users - noniid_img_per_local)
    ...
    local_list = [i for i in range(args.total_users)]

    for k in range(10):
        noniid_to_local[k] =  np.random.choice(local_list, args.total_users//10, replace=False)
